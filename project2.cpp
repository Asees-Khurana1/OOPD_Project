...
private:
    ...
    void change_directory(const vector<string>& input_tokens){
        ...
        if (destination == "-") {
            ...
            // Handle relative path: Combine with current directory
    ...
    void printChangeDirectoryHelp() {
        ...
        cout << "Usage: cd <directory> [options]" << endl;
        cout << "Options:" << endl;
        cout << "  -h, --help     Display this help message." << endl;
        // Add any additional options or information as needed
    }
    ...
    void printMoveFileHelp() {
        ...
        cout << "Usage: mv <source> <destination> [options]" << endl;
        cout << "Options:" << endl;
        cout << "  -h, --help     Display this help message." << endl;
        // Add any additional options or information as needed
    }
    ...
    void removeFile(const vector<string>& tokens) {
        ...
        if (tokens.size() < 2) {
            cout << "Usage: rm <file> [options]" << endl;
            return;
        }
        ...
        if (force) {
            ...
            if (removeMultithreaded(file, recursive, verbose) == 0) {
                cout << "Removed " << file << "." << endl;
            }
        }
    ...
    int removeMultithreaded(const string& file, bool recursive, bool verbose) {
        ...
        if (recursive) {
            // Add logic to identify files to remove recursively
            // Populate the filesToRemove vector accordingly
        } else {
            filesToRemove.push_back(file);
        }

        // Calculate the chunk size for each thread
        size_t chunkSize = filesToRemove.size() / numCores;

        // Vector to store thread objects
        vector<thread> threads;

        // Function to be executed by each thread
        auto removeThread = [this, &filesToRemove, recursive, verbose](size_t start, size_t end) {
        for (size_t i = start; i < end; ++i) {
        const string& file = filesToRemove[i];
        if (this->remove_func(file.c_str(), recursive, verbose) == 0) {
            cout << "Removed " << file << "." << endl;
        }
    }
};
        ...
        for (unsigned int i = 0; i < numCores; ++i) {
            ...
            size_t end = (i == numCores - 1) ? filesToRemove.size() : start + chunkSize;

            threads.emplace_back(removeThread, start, end);
        }
    ...
    void printListDirectoryHelp() {
        ...
        cout << "Usage: ls [directory] [options]" << endl;
        cout << "Options:" << endl;
        cout << "  -h, --help     Display this help message." << endl;
        // Add any additional options or information as needed
    }
    ...
    void copyFile(const vector<string>& tokens) {
        ...
        if (tokens.size() < 3) {
            cout << "Usage: cp <source> <destination> [options]" << endl;
            return;
        }
        ...
        // Function to be executed by each thread
        auto copyThread = [&source, &destination](size_t start, size_t end) {
            char buffer[1024];
            size_t bytesRead;
            ...
            while (start < end) {
                ...
                bytesRead = min(sizeof(buffer), end - start);
        ...
        for (unsigned int i = 0; i < numCores; ++i) {
            size_t start = i * chunkSize;
            size_t end = (i == numCores - 1) ? fileSize : start + chunkSize;

            threads.emplace_back(copyThread, start, end);
        }
